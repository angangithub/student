...
import com.sitech.bds.mp.mapper.StudentsMapper;
...
public class StudentsServiceImpl extends ServiceImpl<StudentsMapper, StudentsEntity> implements StudentsService {
     ...
     */
    @Override
    public Result saveStudents(StudentsEntity students) {
        try{
            Assert.notNull(students,"参数异常");
            boolean flag = this.save(students);
            if (flag) return Result.ok("添加成功");
            throw new Exception("添加失败");
        }catch(Exception e){
            log.error("StudentsService saveStudents ERROR => {}" , e.getMessage());
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @Override
    public Result updateStudents(StudentsEntity students) {
        try{
            Assert.notNull(students,"参数异常");
            boolean flag = this.updateById(students);
            if (flag) return Result.ok("修改成功");
            throw new Exception("修改失败");
        }catch(Exception e){
            log.error("StudentsService updateStudents ERROR => {}" , e.getMessage());
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @Override
    public Result deleteStudents(Long id) {
        try{
            ...
            log.error("StudentsService deleteStudents ERROR => {}" , e.getMessage());
            return Result.error(e.getMessage());
        }
    }
