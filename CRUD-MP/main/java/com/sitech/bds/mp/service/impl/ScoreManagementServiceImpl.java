...
import com.sitech.bds.mp.mapper.ScoreManagementMapper;
...
public class ScoreManagementServiceImpl extends ServiceImpl<ScoreManagementMapper, ScoreManagementEntity> implements ScoreManagementService {
    ...
    public Result pagination(Integer current,Integer size) {
        try{
            ...
            QueryWrapper<ScoreManagementEntity> wrapper = new QueryWrapper<>();
     ...
     */
    @Override
    public Result saveScoreManagement(ScoreManagementEntity scoreManagement) {
        try{
            Assert.notNull(scoreManagement,"参数异常");
            boolean flag = this.save(scoreManagement);
            if (flag) return Result.ok("添加成功");
            throw new Exception("添加失败");
        }catch(Exception e){
            log.error("ScoreManagementService saveScoreManagement ERROR => {}" , e.getMessage());
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @Override
    public Result updateScoreManagement(ScoreManagementEntity scoreManagement) {
        try{
            Assert.notNull(scoreManagement,"参数异常");
            boolean flag = this.updateById(scoreManagement);
            if (flag) return Result.ok("修改成功");
            throw new Exception("修改失败");
        }catch(Exception e){
            log.error("ScoreManagementService updateScoreManagement ERROR => {}" , e.getMessage());
            return Result.error(e.getMessage());
        }
    }
     ...
     */
    @Override
    public Result deleteScoreManagement(Long id) {
        try{
            ...
            log.error("ScoreManagementService deleteScoreManagement ERROR => {}" , e.getMessage());
            return Result.error(e.getMessage());
        }
    }
